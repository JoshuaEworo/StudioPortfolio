<script lang="ts">
    ...
    import { editBlog } from '../../../blogStore';
    ...
    const editPost = async () => {
        await editBlog(id, title, description, imgLink, isPublished);
        title = "";
        description = "";
        imgLink = "";
        isPublished = true;
    ...
    <div transition:fade={{ delay: 250, duration: 100 }} class="viewFill">
        <div class="modal">
            ...
            <div class="form">
                ...
                <div class="published">
                    <label for="link"><p>Published:</p></label>
                    <button on:click={() => {isPublished = !isPublished}}>
                        {#if isPublished}
                            <Fa icon={faToggleOn} size='2.5x'/>
                        {:else}
                            <Fa icon={faToggleOff} size='2.5x'/>
                        {/if}
                    </button>
                </div>
                <div class="submit">
                    <button on:click={editPost} class="submitBtn">Save Post</button>
                </div>
            </div>
        </div>
    </div>
